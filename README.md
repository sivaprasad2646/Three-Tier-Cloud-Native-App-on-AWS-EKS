...
This project implements a production-grade monitoring and alerting solution using **Prometheus**, **Alertmanager**, **Grafana**, and **Slack**.

Prometheus continuously collects metrics from the Kubernetes cluster and application, evaluates custom alert rules, and forwards active alerts to Alertmanager. Alertmanager performs severity-based routing and delivers notifications to dedicated Slack channels.
...
```text
        ...
        severity=warning                 severity=critical
                 │                                 │
                 ▼                                 ▼
          Slack #alerts               Slack #critical-alerts
```
...
The following custom alert rules were implemented to monitor the TaskManager application:

| Alert Name | Severity | Description |
|------------|----------|-------------|
| TaskManagerPodDown | Critical | Triggers when the backend deployment remains unavailable for more than 2 minutes. |
| HighErrorRate | Critical | Detects excessive HTTP 5xx responses from the application. |
| HighMemoryUsage | Warning | Triggers when container memory usage exceeds 85% of the configured memory limit. |
| SlowAPIResponse | Warning | Triggers when the 95th percentile API response latency exceeds one second. |
| NoRequestsReceived | Warning | Detects when the application receives no requests for five consecutive minutes. |

---

## 🔀 Severity-Based Alert Routing

Alertmanager is configured to route alerts based on the `severity` label.

| Severity | Receiver | Notification Destination |
|----------|----------|--------------------------|
| Warning | Slack | `#alerts` |
| Critical | Slack | `#critical-alerts` |

This routing strategy separates high-priority production incidents from lower-priority operational alerts, making alert management more efficient.

---

## 🔄 Alert Lifecycle

Each alert follows the complete Prometheus alert lifecycle:
...
Alertmanager
        │
        ▼
Routes Alert Based on Severity
        │
        ▼
Slack Notification
```
...
The monitoring pipeline was validated by intentionally generating application failures and verifying the complete alert workflow.

### Critical Alert Validation

- Scaled the backend deployment to zero replicas.
- Verified the alert transitioned from **Pending** to **Firing**.
- Confirmed Alertmanager received the alert.
- Verified the notification was routed to the **#critical-alerts** Slack channel.

### Warning Alert Validation

- Triggered warning-level alerts using application metrics.
- Verified Prometheus evaluated the alert successfully.
- Confirmed Alertmanager routed the alert based on its severity.
- Verified warning notifications were delivered to the **#alerts** Slack channel.
...
- Severity-based alert routing
- Separate Slack channels for warning and critical alerts
- Configurable alert grouping and repeat intervals
- Automatic resolved alert notifications (`send_resolved: true`)
...
- Designed and implemented custom Prometheus alert rules.
- Configured Alertmanager for severity-based notification routing.
- Integrated Slack notifications using Incoming Webhooks.
- Validated the complete alert lifecycle from **Pending** to **Firing**.
- Implemented end-to-end monitoring for Kubernetes workloads.
- Verified alert delivery from Prometheus to Alertmanager and Slack.

---

## 💬 Interview Explanation

This project demonstrates a production-style monitoring and alerting pipeline for Kubernetes applications.

Prometheus continuously scrapes metrics from the application and Kubernetes cluster, evaluates custom alert rules, and forwards active alerts to Alertmanager. Alertmanager routes alerts based on their severity labels, delivering warning alerts to the **#alerts** Slack channel and critical alerts to the **#critical-alerts** Slack channel. This implementation demonstrates a scalable alerting architecture that can easily be extended to enterprise notification platforms such as PagerDuty, Microsoft Teams, or Opsgenie without modifying the alert rules themselves.
